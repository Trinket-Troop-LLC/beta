...
export function BetaBottomNav({
  ...
  return (
    ...
    >
      <div className="flex h-16 w-full max-w-md justify-between gap-1 rounded-full border border-border bg-card/95 px-3 shadow-lg backdrop-blur sm:gap-2 sm:px-5">
        {tabs.map(({ id, label, icon, iconActive, href }) => {
          ...
          return (
            <Link
              ...
              className="relative flex min-w-0 flex-1 flex-col items-center justify-end rounded-full pb-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
              aria-current={isActive ? "page" : undefined}
            >
              <span className="relative -mt-8 flex size-14 items-center justify-center drop-shadow-md transition hover:scale-105 sm:size-16">
                <Image
