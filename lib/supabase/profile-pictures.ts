...
const BUCKET = 'beta-profile-pictures'
...
async function signCached(db: Db, path: string): Promise<string | null> {
    const getSignedUrl = unstable_cache(
        ...
        ['beta-profile-picture-signed-url', path],
        { revalidate: CACHE_REVALIDATE_SECONDS }
    )
    return getSignedUrl(path)
}

/** Signs a single profile picture path, reusing a cached URL when one is still fresh. */
export async function signProfilePictureUrl(db: Db, path: string | null | undefined): Promise<string | null> {
    if (!path) return null
    return signCached(db, path)
}

/**
 * Signs a batch of profile picture paths, keyed by path so callers with
 * overlapping paths (e.g. two pages showing the same user) share cache hits.
 */
export async function signProfilePictureUrls(
    db: Db,
